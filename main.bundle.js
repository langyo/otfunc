...
var override = function override(list) {
  ...
  var functions = list.map(function (func, index) {
    ...
    list.forEach(function (n) {
      if (!repeat) return;
      var length = func.parameters.length;
      if (length !== n.parameters.length) return;

      for (var i = 0; i < length; ++i) {
        if (func.parameters[i] instanceof Type) {
          if (n.parameters[i] instanceof Type) repeat = func.parameters[i].equals(n.parameters[i]);else repeat = false;
        } else {
          if (_typeof(func.parameters[i]) !== _typeof(n.parameters[i])) repeat = false;
        }
      }
    });
    ...
    var candidated = functions.filter(function (f) {
      ...
      try {
        for (var _iterator = f.parameters[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          if (!_param instanceof Type && !args[pos] instanceof _param) return false;
          ...
          switch (_param.name) {
            ...
            case "duck":
            case "any":
              if (!_param.match(args[pos])) return false;
              flag[pos++] = true;
              break;

            case "required":
              if (_param.match(args[pos])) flag[pos++] = true;
              break;

            case "default":
              if (_param.match(args[pos])) flag[pos++] = true;else {
                flag[pos++] = true;
                args = args.slice(0, pos).concat([_param.value]).concat(args.slice(pos));
              }
              break;

            case "endless":
              while (_param.match(args[pos])) {
                flag[pos++] = true;
              }
            ...
            default:
              throw new Error("Unknown type: " + _param.name);
          }
        }
      } catch (err) {
...
var typical = function typical(params, func, level) {
  ...
  if (Array.isArray(params)) params = transform_array(param);else if (_typeof(params) === 'object') params = transform_object(param); // Write to the functional object.
...
},{"./types/any":2,"./types/array":3,"./types/default":4,"./types/duck":5,"./types/endless":6,"./types/enum":7,"./types/except":8,"./types/required":9,"./types/union":11}],2:[function(require,module,exports){
"use strict";
